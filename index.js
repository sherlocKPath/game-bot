...
const handleEvent = async (event) => {
  ...
  // ฟังก์ชันถามเกี่ยวกับเกมที่น่าสนใจในช่วงนี้
  if (
    userMessage.includes("เกมอะไรน่าสนใจ") ||
    userMessage.includes("ช่วงนี้มีเกมอะไร")
  ) {
    userState[userId] = "waiting_for_genre"; // ตั้งสถานะให้รอแนวเกม
    return client.replyMessage(event.replyToken, {
      type: "text",
      text: "คุณชอบเกมแนวไหนครับ (เช่น Action, RPG, Adventure)?",
    });
  }

  // ฟังก์ชันตอบเมื่อผู้ใช้เลือกแนวเกม
  if (userState[userId] === "waiting_for_genre") {
    const genre = userMessage.trim(); // รับข้อมูลแนวเกมที่ผู้ใช้ตอบ
    const recommendations = await recommendGamesByGenre(genre); // เรียกใช้ฟังก์ชันแนะนำเกม
    userState[userId] = null; // รีเซ็ตสถานะหลังจากแนะนำเกมเสร็จ
    return client.replyMessage(event.replyToken, {
      type: "text",
  ...
  return client.replyMessage(event.replyToken, {
    type: "text",
    text: "พิมพ์ 'แนะนำเกม [ชื่อเกม]' เพื่อค้นหาข้อมูลเกม หรือพิมพ์ 'เกมยอดนิยมปีนี้' เพื่อรับคําแนะนำเกม 10 อันดับที่นิยมสูงสุดในปีนี้ หรือถามว่า 'มีเกมอะไรน่าสนใจบ้างในช่วงนี้' เพื่อรับคำแนะนำเกม",
  });
};
...
const recommendGamesByGenre = async (genre) => {
  // แมปแนวเกมของผู้ใช้กับแนวเกมที่ API ยอมรับ
  const genreMap = {
    action: "action",
    rpg: "role-playing-games-rpg",
    adventure: "adventure",
    fps: "shooter",
    moba: "multiplayer-online-battle-arena",  // ปรับให้รองรับ MOBA
    strategy: "strategy",
  ...
  const mappedGenre = genreMap[genre.toLowerCase()];

  if (!mappedGenre) return `ขออภัย ไม่รองรับแนวเกม "${genre}" ในตอนนี้ 😔`;

  try {
    const response = await axios.get(`https://api.rawg.io/api/games`, {
      params: {
        key: RAWG_API_KEY,
        genres: mappedGenre, // ใช้ชื่อแนวเกมที่แมปได้
        ordering: "-rating",
